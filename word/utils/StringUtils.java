...
//    	 String MOBILE = "^1\\d{10}$";
	...
	public static boolean isNewVersion(String oldVs, String newVs) {
		if (oldVs == null || newVs == null) {
			return false;
		}
		LogUtil.e("qiansheng", "isNew....." + oldVs + newVs);
		
		boolean flag = false;
		String oldArray[] = oldVs.split("\\.");
		String newArray[] = newVs.split("\\.");
		LogUtil.e("qiansheng", "isNew....." + oldArray.length + newArray.length);
		if (oldArray.length == 3 && newArray.length == 3) {
			// 新的版本号 xx.yy.zz
			for (int i = 0; i < oldArray.length; i++) {
				if (Integer.parseInt(newArray[i]) > Integer
						.parseInt(oldArray[i])) {
					flag = true;
					break;
				} else {
	...
	//判断是否是以前的版本格式  X100000000000
    public static boolean isXVersion(String verSionStr) {
